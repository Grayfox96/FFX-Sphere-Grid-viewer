...
from .data.layout import LAYOUT_EXPERT, LAYOUT_ORIGINAL, LAYOUT_STANDARD, Layout
from .logger import UIHandler, log_exceptions, log_tkinter_error
from .tkspheregrid import KEY_TO_APPEARANCE_TYPE, KEY_TO_CHAR_NAME, TkSphereGrid
    ...
    *,
    ...
    buttons.extend(
        [
            (
                "<F6>",
                lambda _=None: canvas.draw_layout(LAYOUT_ORIGINAL),
                "Original",
            ),
            (
                "<F7>",
                lambda _=None: canvas.draw_layout(LAYOUT_STANDARD),
                "Standard",
            ),
            (
                "<F8>",
                lambda _=None: canvas.draw_layout(LAYOUT_EXPERT),
                "Expert",
            ),
        ]
    )
